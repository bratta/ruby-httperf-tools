...
VALID_OPTIONS = ['server', 'rate', 'port', 'connections', 'send_buffer',
  'recv_buffer', 'uri_list', 'httperf', 'host', 'username', 'password',
  'num_call', 'hog']
...
class HttperfRunner
  ...
  def run_httperf
    @options.uri_list.each do |uri|
      ...
      cmd << "--rate=#{@options.rate} --send-buffer=#{@options.send_buffer} "
